...
public interface CuentaDao extends JpaRepository<Cuenta, Long> {
    ...
    @Query("SELECT c FROM Cuenta c WHERE LOWER(function('replace', c.name, 'á', 'a')) LIKE LOWER(function('replace', :name, 'á', 'a'))" +
    ...
    Optional<Cuenta> getCuentaByName(@Param("name") String name);
